...
function myFunction() {
			...
			$.ajax({
				...
				success : function(data) {
					...
					if (peoples.length % 10 == 0) {
						page = parseInt(peoples.length / 10);
					} else
						page = parseInt(peoples.length / 10) + 1;
					var th = $("<tr></tr>");
					var th0 = $("<th>序号</th>");
					var th1 = $("<th>姓名</th>");
					var th2 = $("<th>学号</th>");
					...
					table.show();
					for (var i = 0; i < 10; i++) {
						if ((i + (pn - 1) * 10) < peoples.length) {
							var tr = $("<tr></tr>");
							var td0 = $("<td>" + (i + (pn - 1) * 10 + 1)
									+ "</td>");
							var td1 = $("<td>"
									+ peoples[i + (pn - 1) * 10].name + "</td>");
							var td2 = $("<td>" + peoples[i + (pn - 1) * 10].id
									+ "</td>");
							if (peoples[i + (pn - 1) * 10].sex == 1)
								var td3 = $("<td>男</td>");
							else
								var td3 = $("<td>女</td>");
							var td4 = $("<td>" + peoples[i + (pn - 1) * 10].pno
									+ "</td>");
							var td5 = $("<td>"
									+ peoples[i + (pn - 1) * 10].mail + "</td>");
							var td6 = $("<td><a href='userEdit.jsp?no="
									+ peoples[i + (pn - 1) * 10].id + "'>修改</a></td>");
