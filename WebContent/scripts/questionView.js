	...
	$.ajax({
		...
		success : function(data) {
			...
			if (questions.length % 10 == 0) {
				page = parseInt(questions.length / 10);
			} else
				page = parseInt(questions.length / 10) + 1;
			var th = $("<tr></tr>");
			var th0 = $("<th>序号</th>");
			var th1 = $("<th>题号</th>");
			var th2 = $("<th>科目</th>");
			...
			table.show();
			for (var i = 0; i < 10; i++) {
				if ((i + (pn - 1) * 10) < questions.length) {
					var tr = $("<tr></tr>");
					var td0 = $("<td>" + (i + (pn - 1) * 10 + 1) + "</td>");
					var td1 = $("<td>" + questions[i + (pn - 1) * 10].qno
							+ "</td>");
					var td2 = $("");
					if (questions[i + (pn - 1) * 10].subject == 0)
						td2 = $("<td>化学类安全题</td>");
					else if (questions[i + (pn - 1) * 10].subject == 1)
						td2 = $("<td>医学生物类安全题</td>");
					else if (questions[i + (pn - 1) * 10].subject == 2)
						td2 = $("<td>通识类安全题</td>");
					else
						td2 = $("<td>电气类安全题</td>");
					var td3 = $("");
					if (questions[i + (pn - 1) * 10].type == 0)
						td3 = $("<td>  判断题    </td>");
					else if (questions[i + (pn - 1) * 10].type == 1)
						td3 = $("<td>单项选择题</td>");
					else
						td3 = $("<td>单项选择题</td>");
					var stem_sub = questions[i + (pn - 1) * 10].stem.substring(
							0, 15);
					var td4 = $("<td>" + stem_sub + "</td>");
					var td5 = $("<td><a href='qEdit.jsp?no="
							+ questions[i + (pn - 1) * 10].qno
							+ "'>修改</a></td>");
					var td6 = $("<td><a onclick='del(" + (i + (pn - 1) * 10)
							+ ");'>删除</a></td>");
					tr.append(td0);
					tr.append(td1);
					tr.append(td2);
			...
			if (page > 4) {
				for (var i = 0; i < 5; i++) {
					if ((pn + i - 2) <= page && (pn + i - 2) >= 1) {
						var li = $("<li><a onclick=\"to_page(" + (pn + i - 2)
								+ ");\">" + (pn + i - 2) + "</a></li>");
						label.append(li);
					}
				}
